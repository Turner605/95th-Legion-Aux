#define NEW_95TH_P1_ARF_UNIT(name) class 95th_P1_ARF_Unit_##name## : 95th_P1_ARF_Unit_Base {\
    scope = 2;\
	scopecurator=2;\
	displayName=[95th] ARF - ##name##;\
	linkedItems[]={95th_P1_ARF_Helmet_##name##,95th_Vest_##name##,ItemMap,JLTS_clone_comlink,ItemCompass,ItemWatch};\
	respawnLinkedItems[]={95th_P1_ARF_Helmet_##name##,95th_Vest_##name##,ItemMap,JLTS_clone_comlink,ItemCompass,ItemWatch};\
	uniformClass=95th_P1_ARF_Uniform_##name##;\
	hiddenSelectionsTextures[]=\
	{\
		\95th_P1_ARF_Units\Data\Uniforms\95th_P1_ARF_Uniform_##name##_Upper.paa,\
		\95th_P1_ARF_Units\Data\Uniforms\95th_P1_ARF_Uniform_##name##_Lower.paa\
	};\

#define NEW_95TH_P1_ARF_UNIFORM(name) class 95th_P1_ARF_Uniform_##name## : 95th_P1_Uniform_Base {\
    scope = 2;\
    scopeArsenal = 2;\
	displayName=[95th] ARF Uniform - ##name##;\
	class ItemInfo: UniformItem\
	{\
		uniformClass=95th_P1_ARF_Unit_##name##;\
		uniformModel=-;\
		containerClass=Supply150;\
		mass=40;\
	};

#define NEW_95TH_P1_ARF_HELMET(name) class 95th_P1_ARF_Helmet_##name## : 95th_P1_ARF_Helmet_Base {\
    scope = 2;\
    scopeArsenal = 2;\
	displayName=[95th] ARF Helmet - ##name##;\
	hiddenSelectionsTextures[]={\95th_P1_ARF_Units\Data\Helmets\95th_P1_ARF_Helmet_##name##.paa};\

class CfgPatches 
{
    class 95th_P1_ARF_Units
    {
        author="95th Aux Team";
		scope=2;
		scopecurator=2;
        name="95th Legion ARF";
        requiredAddons[] = {
			"A3_Characters_F", 
			"JLTS_characters_CloneArmor",
			"95th_P1_Base_Units"
		};
		units[] = {
			"95th_P1_ARF_Unit_Trooper",
			"95th_P1_ARF_Unit_Sergeant",
			"95th_P1_ARF_Unit_Captain",
			"95th_P1_ARF_Unit_Lieutenant"
		};
    };
}; 

class CfgEditorSubcategories{
	class 95th_ARF{displayName="ARF";};
};

class cfgWeapons 
{ 
    class ItemCore;
    class UniformItem;
    class Uniform_Base: ItemCore
    {
        class ItemInfo;
    };

	class 95th_P1_Uniform_Base;
	class 95th_P1_ARF_Helmet_Base;
    
	NEW_95TH_P1_ARF_UNIFORM(Trooper)};
	NEW_95TH_P1_ARF_UNIFORM(Sergeant)};
	NEW_95TH_P1_ARF_UNIFORM(Captain)};
	NEW_95TH_P1_ARF_UNIFORM(Lieutenant)};

	NEW_95TH_P1_ARF_HELMET(Trooper)};
	NEW_95TH_P1_ARF_HELMET(Sergeant)};
	NEW_95TH_P1_ARF_HELMET(Captain)};
	NEW_95TH_P1_ARF_HELMET(Lieutenant)};
}; 

class CfgVehicles 
{     
	class 95th_P1_ARF_Unit_Base;

	NEW_95TH_P1_ARF_UNIT(Trooper)};
	NEW_95TH_P1_ARF_UNIT(Sergeant)};
	NEW_95TH_P1_ARF_UNIT(Captain)};
	NEW_95TH_P1_ARF_UNIT(Lieutenant)};
};