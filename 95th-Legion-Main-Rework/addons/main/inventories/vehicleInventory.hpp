class TransportMagazines {
	class _xx_AUX_95th_Universal_Pistol_Mag{magazine="AUX_95th_Universal_Pistol_Mag"; count=5;};
	class _xx_AUX_95th_Universal_Rifle_Mag{magazine="AUX_95th_Universal_Rifle_Mag"; count=10;};
	class _xx_AUX_95th_Universal_Heavy_Mag{magazine="AUX_95th_Universal_Heavy_Mag"; count=2;};
	class _xx_AUX_95th_Universal_Sniper_Mag{magazine="AUX_95th_Universal_Sniper_Mag"; count=2;};
	class _xx_AUX_95th_Universal_Shotgun_Mag{magazine="AUX_95th_Universal_Shotgun_Mag"; count=2;};
	class _xx_AUX_95th_BX42_Fuel{magazine="AUX_95th_BX42_Fuel"; count=2;};
};

class TransportWeapons {
	class _xx_AUX_95th_DC15A_plastic{weapon="AUX_95th_DC15A_plastic"; count=2;};
	class _xx_3AS_RPS6_F{weapon="3AS_RPS6_F"; count=4;};
};

class TransportItems {
	class _xx_ACE_packingBandage{name="ACE_packingBandage"; count=15;};
	class _xx_ACE_elasticBandage{name="ACE_elasticBandage"; count=15;};
	class _xx_ACE_morphine{name="ACE_morphine"; count=2;};
	class _xx_ACE_adenosine{name="ACE_adenosine"; count=2;};
	class _xx_ACE_epinephrine{name="ACE_epinephrine"; count=2;};
	class _xx_kat_Painkiller{name="kat_Painkiller"; count=2;};
	class _xx_kat_guedel{name="kat_guedel"; count=2;};
	class _xx_kat_larynx{name="kat_larynx"; count=2;};
	class _xx_ACE_salineIV{name="ACE_salineIV"; count=2;};
	class _xx_ACE_tourniquet{name="ACE_tourniquet"; count=4;};
	class _xx_ACE_splint{name="ACE_splint"; count=4;};
	class _xx_kat_chestSeal{name="kat_chestSeal"; count=2;};
};