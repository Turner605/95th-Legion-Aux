...
if (getText (configfile >> 'CfgVehicles' >> typeOf _obj >> 'moves') != 'CfgMovesMaleSdr') exitWith {
	switch true do {
		...
		case (typeOf _obj isKindOf "OPTRE_FC_Elite_Undersuit"): {
			if (isNil {_obj getVariable "WBK_HaloCustomHp"}) exitWith {_obj setDamage 1; _obj remoteExec ["Flame_Death_Particles",[0,-2] select isDedicated,false];};
			if ((_obj getVariable "WBK_HaloCustomHp") <= 0) exitWith {
				[_obj,selectRandom ["Elite_Death_1","Elite_Death_2","WBK_EliteMainWeap_Death_B"]] remoteExec ["switchMove",0];
				_obj remoteExec ["Flame_Death_Particles",[0,-2] select isDedicated,false];
				_obj setDamage 1;
				[_obj,[0,"\WBK_FlamethrowerSystem\burnedFleshCO.paa"]] remoteExec ["setObjectTexture", 0, false];
			};
		...
		case (typeOf _obj isKindOf "OPTRE_Jackal_base_F"): {
			_obj remoteExec ["Flame_Death_Particles",[0,-2] select isDedicated,false];
			_obj setDamage 1;
			[_obj,selectRandom ["flamethrower_burning_1","flamethrower_burning_2","flamethrower_burning_3","flamethrower_burning_4","flamethrower_burning_5","flamethrower_burning_6","flamethrower_burning_7","flamethrower_tankExplodePre_1"]] remoteExec ["switchMove", 0];
		};
...
switch true do {
	case (stance _obj == "CROUCH"): {[_obj,"flamethrower_burning_4"] remoteExec ["switchMove", 0];};
	case ((stance _obj == "STAND") or !(isNil {_obj getVariable "IMS_ISAI"})): {[_obj,selectRandom ["flamethrower_burning_1","flamethrower_burning_2","flamethrower_burning_3","flamethrower_burning_5","flamethrower_burning_6","flamethrower_burning_7","flamethrower_tankExplodePre_1"]] remoteExec ["switchMove", 0];}
};
[_obj,"Disable_Gesture"] remoteExec ["playActionNow", _obj];
[_obj,{
removeVest _this;
...
Flamethrower_Fired_EH = {
    ...
    _muz = currentMuzzle _unit;

    if ((getNumber(configFile >> "CfgWeapons" >> _weap >> _muz >> "WBK_BurnEm_IsNotFlamethrower") == 1)) exitWith {};
...
if (stance _obj == "STAND") then {
	[_obj, "Acts_Shocked_3"] remoteExec ["switchMove",0];
	[_obj, "Acts_Shocked_3"] remoteExec ["playMoveNow",0];
...
[_obj,selectRandom ["burnttodeath3","burnttodeath6","burnttodeath4"],60] call CBA_fnc_GlobalSay3d;
if (stance _obj == "STAND") then {
	[_obj, selectRandom ["flamethrower_tankExplodePre_1","flamethrower_tankExplodePre_2"]] remoteExec ["switchMove",0];
};
};
...
_cos_hraSunset = (-tan _latitude * tan (asin (sin 23.45 * sin ((360 * dateToNumber _date)-(360*(81+2)/365)))));
...
WBK_Flamethrowers_sunrise = _sunrise;
WBK_Flamethrowers_sunset = _sunset;